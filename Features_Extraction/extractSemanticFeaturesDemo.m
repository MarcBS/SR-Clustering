function [ features, tags_names, scores_complete, tags_complete ] = extractSemanticFeaturesDemo( folder, format, Semantic_params, R_Clustering_params, scores_complete, tags_complete )
...
features_used = R_Clustering_params.features_used;
darknet_path = Semantic_params.darknet_path;
...
if (~precomputed_tags)
    ...
    elseif (features_used == 4)
        ...
        text = regexp(text, '%result_path%', 'split');
        text = strjoin(text, tags_dir);
        text = regexp(text, '%darknet_path%', 'split');
        text = strjoin(text, darknet_path);
        text = regexp(text, '%', 'split');
        text = strjoin(text, '%%');
