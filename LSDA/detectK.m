function [top_boxes, cats_found, cats_ids, all_scores] =  detectK(rcnn_model, rcnn_feat, imname, outname, plot, K_scores)

if(~exist('K_scores', 'var'))
    K_scores = 0;
end
if(~exist('plot', 'var'))
    plot = true;
...
% Get boxes
numbox = 256*4;
boxes = extract_boxes(im, numbox, plot);

% Extract per region features and scores
scores = per_region_features(rcnn_model, rcnn_feat, im, boxes, plot);

% Prune the boxes based on scoresegprs
[top_boxes, cats_ids, ~, all_scores] = prune_boxes(boxes, scores, K_scores, plot);
...
function [top_boxes, cats_ids, top_scores, all_scores] = prune_boxes(boxes, scores, K_scores, plot)
max_numcat = 100;
th = tic;
if(plot)
    fprintf('Prune boxes...');
...
function boxes = extract_boxes(im, numbox, plot)
th = tic;
if(plot)
...
function feat = per_region_features(rcnn_model, rcnn_feat, im, boxes, plot)
if(plot)
